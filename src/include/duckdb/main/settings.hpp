...
struct SchedulerTypeSetting {
    using RETURN_TYPE = string;
    static constexpr const char *Name = "scheduler_type";
    static constexpr const char *Description = "The type of scheduler to use. Options: 'default' (FIFO), 'stride' (priority-based stride scheduler), 'ml' (machine learning scheduler).";
    static constexpr const char *InputType = "VARCHAR";
    static void SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &parameter);
    static void ResetGlobal(DatabaseInstance *db, DBConfig &config);
    static Value GetSetting(const ClientContext &context);
